...
public class CustomQueryResultParser<T> {
	...
	public CustomQueryResultParser(Class<T> type, ObjectMapper mapper) {
		this.type = type;
		this.mapper = mapper;
	}
	...
	private void parseRows(JsonParser jp) throws IOException {
		...
		while (dataField == null && jp.nextToken() == JsonToken.START_OBJECT) {
			CustomQueryResultParser.Row row = jp.readValueAs(CustomQueryResultParser.Row.class);
			if (row.error != null) {
